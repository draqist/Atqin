...
func (app *application) joinCohortHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || userID == "" {
		app.errorResponse(w, http.StatusUnauthorized, "Authentication required to join a track")
		return
	}
	...
	// 4. Find or Create the Cohort (The "Sorting Hat")
	cohort, err := app.models.Social.GetOrCreateCohort(roadmapID, input.Pace)
	if err != nil {
		app.logger.Println(err)
		app.errorResponse(w, http.StatusInternalServerError, "Failed to find or create cohort")
		return
	}

	// 5. Add User to Cohort
	err = app.models.Social.JoinCohort(cohort.ID, userID)
	if err != nil {
		// Check if already joined? (Optional: DB constraints usually handle this)
		app.logger.Println(err)
