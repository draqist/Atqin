    ...
    <div className="min-h-screen bg-[#F8F9FA] pb-20">
      ...
      <div className="bg-white border-b border-slate-200 pt-12 pb-8 px-6 sticky top-0 z-10 shadow-xs rounded-lg">
        <div className="w-full mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="relative w-full md:w-80">
              ...
              <Input
                ...
                className="pl-9 bg-slate-50 border-slate-200 focus-visible:ring-emerald-500 rtl:pr-9 rtl:pl-3"
                value={searchQuery}
          ...
          <div className="flex flex-wrap items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
            <SlidersHorizontal className="w-4 h-4 text-slate-400 mr-2 shrink-0 rtl:ml-2 rtl:mr-0" />
            {categories.map((cat) => (
              <button
                key={cat.id}
              ...
              >
                {/* Assuming generic categories are already handled or dynamic */}
                {cat.label}
              </button>
            ))}
          </div>
      ...
      <div className="mx-auto mt-8">
              ...
              {isFetchingNextPage && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6">
                  {[1, 2, 3, 4].map((i) => (
                    <div
                      key={i}
                      className="h-[200px] rounded-xl border border-slate-200 bg-white p-5 space-y-4"
                    >
        ...
        {reflections?.length === 0 ||
          ...
          (isError && (
            <div className="text-center py-20 border-2 border-dashed border-slate-200 rounded-2xl">
              <p className="text-slate-500 text-center">{t("empty.title")}</p>
            </div>
          ))}
      </div>
    </div>
